...
class Challenge{
    ...
    long long solveEach(string input, int idx){
        ...
        while(i < n){
            if(Utils::isdigit(input[i])){
                ...
                if(temp != "" and winners.find(stoi(temp)) != winners.end()) res++;
                temp = "";
            }
            i++;
        }
        ...
        for(int j = idx + 1; j <= idx + res; j++){
            counts[j] += counts[idx];
        }

        scratches += counts[idx];
        // cout << "Winners in  " << idx << " : " << res << " " << counts[idx] << endl;
        return res;
    }
